...
`gl_FragColor.xyz` as `gl_FragColor.rgb` and it means the same thing. It's just a matter of clarity:
prefer `xyzw` if you're using a vector as a position, and `rgba` if you're using it as a color.
