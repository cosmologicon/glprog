...
`gl.COLOR_BUFFER_BIT` and `gl.DEPTH_BUFFER_BIT` are the first of many, many GLenum values that we'll
encounter. You will recognize them by their all-caps names. These GLenums are passed into methods of
`gl` to specify options. Ultimately, they're just integers. For instance, `gl.COLOR_BUFFER_BIT` is
just `16384`, and you can use `16384` anywhere that `gl.COLOR_BUFFER_BIT` is called for. It's just a
matter of clarity that you shouldn't. `gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT` is a rare instance
where you actually do any operations with a GLenum. Usually you just treat it as an opaque value.
